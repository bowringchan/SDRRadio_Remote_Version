...
public:
	mcs_judgement(osmosdr::source::sptr source, unsigned int vector_length, double centre_freq_1,
		double bandwidth0,unsigned int avg_size,unsigned long phase_number):
...
private:
	...
	void ProcessVector(const float *fft_input, const float* ifft_input,const float* phase_input )
	{
		...
		if(ofstream_fft.is_open()){
			for(int i=0;i<m_vector_length;i++){
				ofstream_fft << fft_input[i] << std::endl;
			}
		}
		ofstream_fft.close();
        exit(0);
        
		m_buffer_phase[m_count++] = phase_input[m_vector_length/2];
		if(m_count < m_phase_number){
			//std::cout << m_count << "/" <<m_phase_number<<std::endl;
			return;
		}
		...
		StartJudgement(&mcs_type,m_buffer_phase);
	...
	*/
	//void StartJudgement(int* mcs_type,float *bands0,float* ifft0, const float* phase0){
	  void StartJudgement(int* mcs_type,const float* phase0){
		/* std::ofstream ofstream_ifft("ifft.txt");
		if(ofstream_ifft.is_open()){
			for(int i=0;i<m_phase_number;i++){
		...
		if(ofstream_phase.is_open()){
			for(unsigned long i=0;i<m_phase_number;i++){
		...
		float z[m_phase_number];//实信号的瞬时幅度
		float ma = 0;//瞬时(绝对)幅度的平均值
		float y2[m_phase_number];//幅度比,即为文献中的an(i)
		float y3[m_phase_number];//归一化瞬时幅度
		double y2n[m_phase_number];//即为文献中的acn(i)
		double s[m_phase_number];
		float y4 = 0;//y3的绝对值的最大值
		double R[m_phase_number];
		double Rmax = 0; //零中心归一化瞬时幅度的谱密度的最大值
		float Ck[m_phase_number];

		const float pi = 3.1415926; // 相位的单位
		const int m = m_phase_number -1;
		float phase1[m_phase_number];
		float phasen1[m_phase_number];
		float phaseabs[m_phase_number];
		float phasesquare[m_phase_number];
		  
		const float at=1; //判决门限电平
		float a =0; //非线性相位之平方的和
		float b = 0; //非线性相位之绝对值的和
		float d = 0; //非线性相位的和
		unsigned long c = 0; //计数器，计算有多少个采样点，幅度的绝对值是大于判决门限的
...
/* 		for(unsigned int i=0; i< m_phase_number; ++i){
			z[i] = std::abs(ifft0[i]);
			ma += (z[i]/m_phase_number);
		}
		for(unsigned int i=0; i< m_phase_number; ++i){
			y2[i] = z[i]/ma;
			y3[i] = y2[i]-1;
		...
		for(unsigned int i=0; i< m_phase_number; ++i){
		  ...
		  //s=fft(y2n);做fft变换
		fftw_plan my_plan;
		my_plan = fftw_plan_r2r_1d(m_phase_number, y2n, s, FFTW_R2HC, 0);
		fftw_execute(my_plan);
		
		fftw_destroy_plan(my_plan);
		  //std::cout << "Step out here: fftw_plan_r2r_1d" << std::endl;
		//s数组的格式:r0, r1, r2, ..., rn/2, i(n+1)/2-1, ..., i2, i1; r是实部,i是虚部
		for(unsigned int i=0; i< m_phase_number/2; ++i){
			R[i] = std::abs(s[i]*s[i]+s[m_phase_number-i]*s[m_phase_number-i]); //此处和matlab代码不同，没有开根号
			 if(R[i] > Rmax){
				Rmax = R[i];
			}
		}
		...
		for(unsigned int i=1; i< m_phase_number-1; ++i){
		  ...
		  for(unsigned long i =0; i< m_phase_number; ++i){
			  //if(y2[i] > at){
				  //c=c+1;
				  phasesquare[i] = phase0[i]*phase0[i]; //非线性相位的平方
				  a=a+phasesquare[i]; //非线性相位之平方的和
				  phaseabs[i]=std::abs(phase0[i]); //非线性相位的绝对值
				  b=b+phaseabs[i]; //非线性相位之绝对值的和
				  d=d+phase0[i];    //非线性相位的和
			  //}
		...
		/*   for(unsigned int i =0; i< m_phase_number/2; ++i){
		  ...
		  for(unsigned int i =m_phase_number/2; i< m_phase_number; ++i){
		     ...
		     double bandwidth0,unsigned int avg_size,unsigned long phase_number)
{
	return boost::shared_ptr<mcs_judgement>(new mcs_judgement(source, vector_length, centre_freq_1,
		     bandwidth0,avg_size,phase_number));
}
